...
public interface MailBoxRepository extends JpaRepository<MailBox,String> {
	...
	/*public MailBox getMailBoxByName(String nome);
	@Query(value ="SELECT m FROM MailBox m join fetch m.account a WHERE m.nome =: nome")
	public MailBox query1(String nome);
*/
	
	@Query(value ="select m from MailBox jetch join m.accountMittente where m.nome=:nome" , nativeQuery = true)
	public List<MailBox> getMailBoxByName(String nome);
	
	
	
	
	@Query(value = "Select m from MailBox m join fetch m.accountMittente.email =: email" , nativeQuery = true)
	public List<MailBox> getMailBoxByEmail(String email);
	
	
	
	
	 @Query(value="Select m from MailBox m where m.accountMittente.email =: email",
	 countQuery = "Select count(m) from MailBox where m.accountMittente.email =: email", nativeQuery = true)
